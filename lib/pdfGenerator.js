...
function drawStudentReportCard(doc, student, subjects, classInfo, testLabel, testType) {
  ...
  // Drawn before header image so it appears behind it
  ...
  // Header Image
  try {
    doc.addImage('/header.png', 'PNG', 10, y, 190, 35);
  } catch (e) {
  ...
  // Table Data setup
  const tableHead = [['Subject', 'Full Marks', 'Pass Marks', 'Marks Scored']];
  const tableBody = [];
  let totalObtained = 0;
  let totalFull = 0;
  let hasFailed = false;
  ...
  const summaryLines = [
    ...
    { label: 'Total Marks Scored', value: `${totalObtained} / ${totalFull}` },
  ...
  doc.text('Parent\'s Signature', 165, y, { align: 'center' });
}
...
function drawCumulativeReportCard(doc, student, subjects, classInfo) {
  const { className, section, academicYear } = classInfo;
  ...
  autoTable(doc, {
    ...
    head: [
      [
        ...
        { content: 'Pass\nMarks', rowSpan: 2, styles: { halign: 'center', valign: 'middle' } },
        { content: 'Marks Scored', colSpan: 3, styles: { halign: 'center' } }
      ],
