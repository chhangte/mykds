      ...
      <div style={{ marginBottom: '0.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '0.8rem' }}>
          {TEST_TABS.filter(t => t.type === 'classtest').map((tab, i) => (
            <button key={i} onClick={() => setActiveTab(i)} style={{ padding: '0.4rem 1rem', borderRadius: 20, background: activeTab === i ? 'var(--sky)' : 'var(--card-bg)', color: activeTab === i ? 'white' : 'var(--text)', fontFamily: 'Inter', fontWeight: activeTab === i ? 600 : 400, fontSize: '0.78rem', cursor: 'pointer', border: activeTab === i ? 'none' : '1px solid var(--border)' }}>{tab.label}</button>
          ))}
        </div>
        ...
        <>
          <div style={{ overflowX: 'auto', borderRadius: 16, border: '1px solid var(--border)', background: 'var(--card-bg)', marginBottom: '1.5rem' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 400 + (data.subjects.length * 100) }}>
              <thead>
                <tr>
                  <th style={{ ...thStyle, minWidth: 80, position: 'sticky', left: 0, zIndex: 20, borderRight: '1px solid var(--border)' }}>Roll No</th>
